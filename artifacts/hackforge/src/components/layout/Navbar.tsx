...
export function Navbar() {
  ...
  if (hasParticipantToken || me?.participantCode) {
    navLinks.push({ href: "/watch", label: "Watch Live", icon: <Tv className="w-4 h-4 mr-2" /> });
  }
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-mono font-bold text-lg tracking-tight">HACK<span className="text-primary">FORGE</span></span>
        </Link>
